...
Tetromino.prototype = {
	first: function(x0, y0, orientation, fn, doBreak) { // fn(x, y), accepts coordinates of each block, returns true to break
		var row = 0, col = 0, result = false, blocks = this.shape.blocks[orientation];
...
var layout = {
	...
	resizeBody: function() {
		...
		var verticalSize = window.innerHeight - 2 * this.spacing.outsize - 2 * this.spacing.inside - 2 * this.spacing.border;
		this.blockSize = Math.floor(verticalSize/gameSizeInBlocks.y);
		var adjustedVerticalSize = this.blockSize * gameSizeInBlocks.y;		
		this.board.style.height = adjustedVerticalSize;
		var boardWidth = this.blockSize * gameSizeInBlocks.x;
		this.board.style.width = boardWidth;
		var upcomingWidth = this.blockSize * this.upcomingPreviewSize;
		...
		var width1 = rendering.promptText.offsetWidth;
		setText(rendering.statusVerb, this.statusWordSet.start);
		var width2 = rendering.promptText.offsetWidth;
		this.left.style.width = maximum(upcomingWidth, width1, width2, upcomingWidth);
		main.style.borderWidth = this.spacing.border;
		main.style.marginTop = (window.innerHeight - main.offsetHeight) / 2;
		main.style.marginLeft = (window.innerWidth - main.offsetWidth) / 2;
...
var game = {
	...
	randomTetromino: function() {
		var chosen = tetrominoSet[randomInt(0, tetrominoSet.length-1)];
	...
	rotate: function() {
		var newOrientation = (this.current.orientation === this.orientation.max ? this.orientation.min : this.current.orientation + 1);
	...
	removeLine: function(n) {
		for(var y = n; y >= 0; --y)
			for(var x = 0; x < gameSizeInBlocks.x; ++x)
 					this.setBlock(x, y, (y === 0) ? null : this.getBlock(x, y-1));
	}, //removeLine

	removeLines: function() {
		var complete, n = 0;
 			for(var y = gameSizeInBlocks.y; y > 0; --y) {
			complete = true;
			for(var x = 0; x < gameSizeInBlocks.x; ++x)
...
var rendering = {
	...
	draw: function() {
		...
		var finerLines = function(context) {
			context.lineWidth = 1;
			context.translate(0.5, 0.5);
		}; //finerLines
		var drawUpcoming = function(context) {
			if (!this.invalid.upcoming) return;
			if (game.states.current != game.states.playing) return;
			var padding = (layout.upcomingPreviewSize - game.next.shape.size) / 2; // half-arsed attempt at centering next tetromino display
			context.save();
			finerLines(context);
		...
		var drawBoard = function(context) {
			...
			var block;
			for(var y = 0; y < gameSizeInBlocks.y; y++)
				for (var x = 0; x < gameSizeInBlocks.x; x++)
		...
		var drawState = function() {
			...
			setText(statusVerb, game.states.current === game.states.paused ? layout.statusWordSet.continue : layout.statusWordSet.start);
